...
function apiRequest($path, $key)
{
	...
	} else if (($resp = json_decode($jsonData)) === false) {
...
try {
	...
	$exitCode = 0;
	$opts = getopt("k:d:f:o:zZqlnwb:u:p:t:c:");
	...
	if (!isset($opts["k"]) || !$opts["k"]) {
		echo "usage: {$argv[0]} -k <accountKey> [-l] [-d <dbType>] [-f <format>] [-o <outputDir|outputFileName>] [-b <dataSourceName> [-u <dbUser>] [-p <dbPassword] [-t <dbTableName>]] [-c <configFile>] [-n] [-z|-Z] [-w] [-q]\n";
	...
	if (isset($db)) {
		...
		$numRows = (int)$db->query("select count(1) cnt from `{$tempTable}`")->fetchObject()->cnt;
		$db->exec("drop table if exists `{$tempTable}_old`");
		$db->exec("rename table `{$dbTable}` to `{$tempTable}_old`, `{$tempTable}` to `{$dbTable}`");
		$db->exec("drop table `{$tempTable}_old`");
